...
Cosmos.catch_fatal_exception do
  ...
  require 'cosmos/tools/param_sender/cmd_sender_text_edit'
  require 'cosmos/tools/param_sender/cmd_sender_item_delegate'
end
...
module Cosmos
  ...
  class CmdSender < QtTool
    ...
    def initialize(options)
      ...
      super(options)
      Cosmos.load_cosmos_icon("nasa.png")
    ...
    def initialize_central_widget
      ...
      # Set the tracker combobox selection
      @track_select = Qt::ComboBox.new
      @track_select.setMaxVisibleItems(2)
      connect(@track_select, SIGNAL('activated(const QString&)'), self, SLOT('target_changed(const QString&)'))
      track_label = Qt::Label.new(tr("&Track:"))
      track_label.setBuddy(@track_select)
      ...
      cmd_label = Qt::Label.new(tr("&Target:"))
      cmd_label.setBuddy(@cmd_select)

      # Button to send command
      send = Qt::PushButton.new("Send All Parameters to GMSEC")
      ...
      #select_layout.addWidget(@track_select, 1) # Move in to table
    ...
    def send_button
      String message = "<MESSAGE SUBJECT=\"GMSEC.COSMOS.MNEMONIC-SENDER.LIST\" KIND=\"PUBLISH\">\n<FIELD TYPE=\"STRING\" NAME=\"COMPONENT\">COSMOS</FIELD>\n<FIELD TYPE=\"STRING\" NAME=\"FACILITY\">NASA-GSFC</FIELD>\n<FIELD TYPE=\"STRING\" NAME=\"MESSAGE-TYPE\">MNEMONIC-LIST</FIELD>\n<FIELD TYPE=\"STRING\" NAME=\"INSTRUMENT\">INST</FIELD>\n" # @ Mike Walker

      get_params.each do |par|
        message = "#{message}<FIELD TYPE=\"STRING\" NAME=\"#{par}\"></FIELD>\n" # @ Mike Walker
      end

      message = "#{message}</MESSAGE>"

      `export DYLD_LIBRARY_PATH=/Users/mwalker8/GMSEC/GMSEC_API/bin && /usr/bin/java -Djava.library.path=/Users/mwalker8/GMSEC/GMSEC_API/bin -cp /Users/mwalker8/GMSEC/GMSEC_API/bin/gmsecapi.jar:/Users/mwalker8/GMSEC/GMSEC_API/examples/java gmpub connectiontype=gmsec_bolt message='#{message}'`
    end
      ...
      if target_name and packet_name
        ...
        pars.each do |par|
          ...
          unless drawn_header
            ...
            @@table.setRowCount(pars.length)
            @@table.setColumnCount(2)
            @@table.setHorizontalHeaderLabels(['Parameters', 'Tracking?'])
    ...
    def self.run (option_parser = nil, options = nil)
      Cosmos.catch_fatal_exception do
        unless option_parser && options
          ...
          options.width = 600
          options.height = 425
          options.title = 'Parameter Sender'
    ...
    def get_params()
      params = Array.new
      oput = ''
      f = File.open("/Users/mwalker8/COSMOS/demo/config/targets/INST/cmd_tlm/inst_tlm.txt", "r")
      f.each_line do |line|
        if ((line.include? "ITEM") and !(line.include? "<%"))
          n = line.index('ITEM')
          n = n + 5
          line = line[n, line.length]
          n = line.index(' ')
          line = line[0, n]
          params.push(line.upcase)
        end
      end
      return params
    end
